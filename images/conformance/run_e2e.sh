...
set -x
ginkgo "${ginkgo_args[@]}" ${E2E_BINARY} -- --disable-log-dump --repo-root=/kubernetes --provider="${E2E_PROVIDER}" --report-dir="${RESULTS_DIR}" --kubeconfig="${KUBECONFIG}" -v="${E2E_VERBOSITY}" > >(tee "${RESULTS_DIR}"/e2e.log)
ret=$?
set -x
saveResults
exit ${ret}
